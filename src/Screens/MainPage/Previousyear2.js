...
const Previousyear2 = ({ navigation,route }) => {
...
const getChildcatdata = async () =>{
    // console.log("item",item.id);
    // setType(item.category_name);
     try{
        const response = await fetch(
          `http://coaching.futurevalue.in/api/previousyearchildcategory/${id.id}`
        );
        const myData = await response.json();

        // console.log("hgduytgdu",myData);
        setChildcatdata(myData.data);
        //  setLoading(false)
      }
      catch(error){
        console.log(error);
 ...
 useEffect(() => {
  ...
  return (
    ...
    <View>
        <Text style={{marginLeft:10,marginTop:-10,fontSize:25,color:'#111',fontWeight:'bold'}}>Previous Year PDF</Text>
        <Text style={{marginLeft:10,marginTop:10,fontSize:16,color:'#111',fontWeight:'bold'}}>{childcatdata[0]?.category_name} / {childcatdata[0]?.sub_category_name}</Text>
   ...
   <ScrollView showsVerticalScrollIndicator = {false}>
     {
        ...
        childcatdata?.map((item) => {
          // console.log('subcatid',item.id)
        return(
          
          <TouchableOpacity style={[styles.dropdownview]}
          onPress={() => navigation.navigate('Previousyear3',{id:item.id})}>
          <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
          <Text style={{marginLeft:20,fontSize:15,color:'#444',fontWeight:'bold'}}>{item.child_category}</Text>
