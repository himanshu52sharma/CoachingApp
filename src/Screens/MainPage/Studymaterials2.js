...
const Studymaterials2 = ({ navigation,route }) => {
    ...
    // setType(item.category_name);
     try{
        const response = await fetch(
          `http://coaching.futurevalue.in/api/studymaterialchildcategory/${id.id}`
        );
 ...
 useEffect(() => {
  ...
  return (
    ...
    <View>
        ...
        <Text style={{marginLeft:10,marginTop:10,fontSize:16,color:'#111',fontWeight:'bold'}}>{childcatdata[0]?.category_name} / {childcatdata[0]?.subcategory_name}</Text>
   ...
   <ScrollView showsVerticalScrollIndicator = {false}>
     {
        ...
        return(
         ...
         onPress={() => navigation.navigate('Studymaterials3')}/>
          </View>
      </TouchableOpacity>
      )}
        )}
...
export default Studymaterials2
