...
const Studymaterials = ({ navigation,route }) => {
  const data = route.params
  console.log('materialcategory',data);
    ...
    // setType(item.category_name);
     try{
        const response = await fetch(
          `http://coaching.futurevalue.in/api/studymaterialsubcategory/${item.id}`
        );
   ...
   <ScrollView showsVerticalScrollIndicator = {false}>
     {
        ...
        return(
          ...
          onPress={() => navigation.navigate('Studymaterials2',{id:item.id})}>
         ...
         onPress={() => navigation.navigate('Studymaterials2')}/>
          </View>
      </TouchableOpacity>
      )}
        )}
