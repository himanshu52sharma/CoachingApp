...
const Practicequestions = ({ navigation,route }) => {
  const data = route.params
  console.log('dknmk',data);
    ...
    setType(item.category_name);
     try{
        const response = await fetch(
          `http://coaching.futurevalue.in/api/practicequestionsubcategory/${item.id}`
        );
        const myData = await response.json();

        // console.log("hgduytgdu",myData);
        setSubcatdata(myData.data);
        //  setLoading(false)
      }
      catch(error){
        console.log(error);
   ...
   const setItems = () => 
   {
     data.data.map((item,index) => {
       return(
        ...
        console.log("hgduytgdu",subcatdata);
 ...
 useEffect(() => {
  ...
  return (
    ...
    <View>
        <Text style={{marginLeft:10,marginTop:-10,fontSize:25,color:'#111',fontWeight:'bold'}}>Practice Question</Text>
     </View>
    ...
    <ScrollView showsHorizontalScrollIndicator={false} horizontal={true}>
    {
        ...
        return(
            <TouchableOpacity
            onPress={() => {setDatas(item);  setType(item.category_name);}}
            style={{
              ...styles.btn,
   ...
   <ScrollView showsVerticalScrollIndicator = {false}>
   {
        ...
        return(
          ...
          onPress={() => navigation.navigate('Queofpracticeque',{id:item.id})}>
          <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
          <Text style={{marginLeft:20,fontSize:15,color:'#444',fontWeight:'bold'}}>{item.subcategory_name}</Text>
          <AntDesign name="right" size={26} color="black" style={[styles.icon1]}
         onPress={() => navigation.navigate('Queofpracticeque')}/>
          </View>
      </TouchableOpacity>
      )}
        )}
