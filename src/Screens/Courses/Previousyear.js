...
const Previousyear = ({ navigation,route}) => {
  const data = route.params
  console.log('previouscat',data);
    ...
    // setType(item.category_name);
     try{
        const response = await fetch(
          `http://coaching.futurevalue.in/api/previousyearsubcategory/${item.id}`
        );
   ...
   <ScrollView showsVerticalScrollIndicator = {false}>
     {
        ...
        return(
          ...
          onPress={() => navigation.navigate('Previousyear2',{id:item.id})}>
          <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center'}}>
          <Text style={{marginLeft:20,fontSize:15,color:'#444',fontWeight:'bold'}}>{item.sub_category_name}</Text>
